...
本文档只描述v0.1.0的MVP架构。代码级接口和字段细节将在实现过程中由Go类型、OpenAPI文档和测试进一步固定。
...
| Nginx | v0.1.0核心范围不启用，验收完成后可选 | 本文档 |
...
- 后续复杂迁移再引入显式迁移版本工具，v0.1.0不提前增加额外系统。
...
- `.env`和未来证书不进入Git。
- v0.1.0内部网络不引入复杂认证，但架构保留后续内部令牌或mTLS扩展点。
...
Filebeat和Logstash使用各自运行日志观察输入启动、连接、队列积压和重试。v0.1.0不引入Prometheus，但不得阻碍后续增加指标端点。
